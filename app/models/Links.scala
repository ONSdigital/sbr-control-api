...
object Links {
  ...
  def toCC(x: List[StatisticalUnit]): List[Links] = x.map(u => Links(u.getKey, toStringMap(u.getLinks.getParents.toMap),
    u.getLinks.getChildJsonString, u.getType.toString))
