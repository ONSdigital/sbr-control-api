...
class UnitLinksPatchServiceSpec extends FreeSpec with Matchers with MockFactory with ScalaFutures {
  ...
  private trait Fixture {
    ...
    val service = new UnitLinksPatchService(repository, unitRegisterService)
  }
  ...
  "A UnitLinksPatchService" - {
    "can apply a patch that specifies a checked update of a VAT unit's parent UBRN to a Legal Unit that already exists in the register" in new Fixture {
      (unitRegisterService.isRegisteredUnit _).expects(TargetLegalUnitId, LegalUnit, RegisterPeriod).returning(
        Future.successful(UnitFound)
      )
      (repository.updateParentId _).expects(VatUnitId, ValueAddedTax, RegisterPeriod, LegalUnit, IncorrectLegalUnitId, TargetLegalUnitId).returning(
        Future.successful(UpdateApplied)
      )

      whenReady(service.applyPatchTo(VatUnitId, ValueAddedTax, RegisterPeriod, UpdateParentPatch)) { result =>
    ...
    "rejects a patch for any path other than '/parents/LEU'" in new Fixture {
      ...
      whenReady(service.applyPatchTo(VatUnitId, ValueAddedTax, RegisterPeriod, invalidPathPatch)) { result =>
    ...
    "rejects a patch that does not contain both a 'test' and a 'replace' operation" in new Fixture {
      ...
      whenReady(service.applyPatchTo(VatUnitId, ValueAddedTax, RegisterPeriod, noTestPatch)) { result =>
        result shouldBe PatchRejected
      }
    }
    ...
    "rejects a patch containing a test value that is not a string and so cannot be a UBRN" in new Fixture {
      ...
      whenReady(service.applyPatchTo(VatUnitId, ValueAddedTax, RegisterPeriod, invalidUbrnPatch)) { result =>
        result shouldBe PatchRejected
      }
    }
